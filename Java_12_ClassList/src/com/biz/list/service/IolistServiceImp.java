...
import com.biz.list.exec.model.IolistVO;
...
public class IolistServiceImp {
	...
	public void input(int count) {
		...
		for(int i = 0 ; i < count ; i++) {
			...
			/*
			 * 매개변수가 5개인 생성자를 이용해서 객체생성
			 */
			iolistVO 
			= new IolistVO("2019-09-05", strTime, strPname, 1000 * intH, intQty);
			iolist.add(iolistVO);
		...
		public void total() {
			...
			// 확장된 for를 이용하면 
			// setter method가 제대로 작동하지 않는 경우도 있다
			for(IolistVO vo : iolist) {
				int total = vo.getIntPrice() * vo.getIntQty();
				vo.setIntTotal(total);
			}
			
			// setter method를 사용할 경우에
			// setter method가 제대로 작동하지 않으면
			// 고전 for를 사용해야 한다.

...
public class GradeServiceV1 implements GradeService {
	...
	public void makeScoeFile(String fileName, int length) throws Exception {
		
		Random rnd = new Random();
		
		/*
		 * 매개변수로 전달받은 length 만큼
		 * score정보를 생성하고 scList에 추가해 보기
		 */
		...
		for(ScoreVO vo : scList) { 
			...
			// A0001:90:80:70 형식으로 작성하겠다는 소리
			fileOut.printf("%s:%d:%d:%d\n", vo.getNum(), vo.getKor(), vo.getEng(), vo.getMath());
			
			// 현재의 데이터를 안전하게 기록하기 위해서
			// 한 사람의 성적을 기록한 후 파일에 강제 저장하도록 flush를 실행함.
			// (데이터가 많을 때는 성능상에 불리한 면이 있을 수 있음.)
			fileOut.flush();
		}
		fileOut.close();

...
public class GradeServiceImp_01 implements GradeService {
	...
	public void total() {
		...
		for(int i = 0 ; i < nSize; i++) {
			...
			// get(i) = gradeList의 i번재에 저장되어있는 
			// gVO의 intKor 값을 읽어서
			// total 변수에 복사해라
			int total = gradeList.get(i).intKor;
			
			// gradeList의 i번째에 저장되어있는 intEng의 값을 읽어서
			// total에 저장된 값과 더하여 다시 total에 저장하라
			total += gradeList.get(i).intEng;
			total += gradeList.get(i).intMath;
			
			
			// gradeList의 i번째에 저장되어있는
			// gVo intTotal 멤버변수에
			// total 변수에 저장된 값을 복사하라
			gradeList.get(i).intTotal = total;
			
			// gradeList의 i번째에 저장되어 있는
			// gVo intAvg 멤버변수에

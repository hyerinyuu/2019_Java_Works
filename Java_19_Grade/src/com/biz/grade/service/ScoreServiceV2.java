...
public class ScoreServiceV2 {
	...
	public void rank() {
		...
		// [general]
		// List<general> : List가 어떤 type의 데이터들을 가지고 있을 것인가를 명시
		
		// *******************************
		// map 형태를 List로 변환시킬때는
		// =======>  List<Map.Entry<>> 형태로 선언해주어야함.
		List<Map.Entry<String, ScoreVO>> sortList 
			= new LinkedList<Map.Entry<String, ScoreVO>>(scList.entrySet());
		
		
		// Collections.sort는 반복문
		// sortList에 있는 값을 하나씩 compare로 보내서 비교하고 정렬한 값을 저장함.
		Collections.sort(sortList, new Comparator<Map.Entry<String, ScoreVO>>() {
			...
			public int compare(Entry<String, ScoreVO> o1, Entry<String, ScoreVO> o2) {
				...
				// o2.getValue()  ==> list형태로 바뀐 Map에서 VO를 추출하는 method
				// o1 - o2 의 값이 +1이면 o2가 더 크다는 소리이므로 두 값의 위치를 바꿔버림(0나 -1이면 그대로) ==> 내림차순정렬
